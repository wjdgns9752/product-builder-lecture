...
async function analyzeNoiseCharacteristics() {
    ...
    // Reset stuck processing (Fail fast after 1s)
    if (isModelProcessing) {
        if (window.lastModelStartTime && Date.now() - window.lastModelStartTime > 1000) {
    ...
    try {
        ...
        // Execute Model with Race Timeout (1s limit for real-time feel)
